...
static int callbinTM(lua_State *L, const TValue *p1, const TValue *p2, StkId res,
                     TMS event) {
...
void luaT_trybinassocTM(lua_State *L, const TValue *p1, const TValue *p2, int flip,
                        StkId res, TMS event) {
  if (flip)
...
void luaT_trybiniTM(lua_State *L, const TValue *p1, lua_Integer i2, int flip, StkId res,
                    TMS event) {
  TValue aux;
  setivalue(&aux, i2);
...
int luaT_callorderiTM(lua_State *L, const TValue *p1, int v2, int flip, int isfloat,
                      TMS event) {
  TValue aux;
  const TValue *p2;
  if (isfloat) {
...
**                                          ^ ci->func
*/
static void buildhiddenargs(lua_State *L, CallInfo *ci, const Proto *p, int totalargs,
                            int nfixparams, int nextra) {
  int i;
...
*/
void luaT_getvarargs(lua_State *L, CallInfo *ci, StkId where, int wanted, int vatab) {
