...
static void LUAI_TRY(lua_State *L, lua_longjmp *c, Pfunc f, void *ud) {
  try { f(L, ud); /* call function protected */ }
  catch(lua_longjmp * c1) { /* Lua error */
    if (c1 != c)            /* not the correct level? */
      throw;                /* rethrow to upper level */
  }
  catch(...) { /* non-Lua exception */ c->status = -1; /* create some error code */ }
}
...
*/
l_sinline void moveresults(lua_State *L, StkId res, int nres, l_uint32 fwanted) {
...
void luaD_poscall(lua_State *L, CallInfo *ci, int nres) {
  ...
  lua_assert(!(ci->callstatus & (CIST_HOOKED | CIST_YPCALL | CIST_FIN | CIST_CLSRET)));
...
*/
l_sinline CallInfo *prepCallInfo(lua_State *L, StkId func, unsigned status, StkId top) {
...
TStatus luaD_protectedparser(lua_State *L, ZIO *z, const char *name, const char *mode) {
  struct SParser p;
  TStatus status;
