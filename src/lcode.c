...
static void savelineinfo(FuncState *fs, Proto *f, int line) {
  ...
  if (abs(linedif) >= LIMLINEDIFF || fs->iwthabs++ >= MAXIWTHABS) {
    luaM_growvector(fs->ls->L, f->abslineinfo, fs->nabslineinfo, f->sizeabslineinfo,
                    AbsLineInfo, INT_MAX, "lines");
...
int luaK_codevABCk(FuncState *fs, OpCode o, int A, int B, int C, int k) {
  ...
  lua_assert(A <= MAXARG_A && B <= MAXARG_vB && C <= MAXARG_vC && (k & ~1) == 0);
...
void luaK_indexed(FuncState *fs, expdesc *t, expdesc *k) {
  ...
  if (k->k == VKSTR)
    keystr = str2K(fs, k);
  lua_assert(!hasjumps(t) && (t->k == VLOCAL || t->k == VVARGVAR || t->k == VNONRELOC ||
                              t->k == VUPVAL));
...
*/
static void finishbinexpval(FuncState *fs, expdesc *e1, expdesc *e2, OpCode op, int v2,
...
*/
static void codebini(FuncState *fs, OpCode op, expdesc *e1, expdesc *e2, int flip,
                     int line, TMS event) {
...
*/
static void codebinK(FuncState *fs, BinOpr opr, expdesc *e1, expdesc *e2, int flip,
                     int line) {
...
*/
static int finishbinexpneg(FuncState *fs, expdesc *e1, expdesc *e2, OpCode op, int line,
                           TMS event) {
  if (!isKint(e2))
...
*/
static void codearith(FuncState *fs, BinOpr opr, expdesc *e1, expdesc *e2, int flip,
                      int line) {
