...
static void trymt(lua_State *L, const char *mtkey, const char *opname) {
  ...
  if (l_unlikely(lua_type(L, 2) == LUA_TSTRING || !luaL_getmetafield(L, 2, mtkey)))
    luaL_error(L, "attempt to %s a '%s' with a '%s'", opname, luaL_typename(L, -2),
               luaL_typename(L, -1));
...
*/
static ptrdiff_t get_onecapture(MatchState *ms, int i, const char *s, const char *e,
                                const char **cap) {
  if (i >= ms->level) {
    if (l_unlikely(i != 0))
...
*/
static int add_value(MatchState *ms, luaL_Buffer *b, const char *s, const char *e,
                     int tr) {
  lua_State *L = ms->L;
  switch (tr) {
...
static int lua_number2strx(lua_State *L, char *buff, unsigned sz, const char *fmt,
                           lua_Number x) {
...
** despite its size.
*/
static KOption getdetails(Header *h, size_t totalsize, const char **fmt, size_t *psize,
                          unsigned *ntoalign) {
...
static int str_pack(lua_State *L) {
  ...
  while (*fmt != '\0') {
    ...
    luaL_argcheck(L, size + ntoalign <= MAX_SIZE - totalsize, arg, "result too long");
    ...
    case Kuint: { /* unsigned integers */
      lua_Integer n = luaL_checkinteger(L, arg);
      if (size < SZINT) /* need overflow check? */
        luaL_argcheck(L, (lua_Unsigned)n < ((lua_Unsigned)1 << (size * NB)), arg,
                      "unsigned overflow");
    ...
    case Kstring: { /* strings with length count */
      ...
      const char *s = luaL_checklstring(L, arg, &len);
      luaL_argcheck(
          L, size >= sizeof(lua_Unsigned) || len < ((lua_Unsigned)1 << (size * NB)),
...
static int str_packsize(lua_State *L) {
  ...
  while (*fmt != '\0') {
    ...
    luaL_argcheck(L, totalsize <= MAX_SIZE - size, 1, "format result too large");
    totalsize += size;
  }
