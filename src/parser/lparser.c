...
/*
** Function body for class methods.
** Supports both '{...}' and 'end' styles as terminators.
*/
static void classfuncbody (LexState *ls, expdesc *e, int line) {
  FuncState new_fs;
  BlockCnt bl;
  new_fs.f = addprototype(ls);
  new_fs.f->linedefined = line;
  ...
  parlist(ls);
  checknext(ls, ')');
  if (ls->t.token == '{') {  /* brace-style body? */
    luaX_next(ls);  /* skip '{' */
...
static void classstat (LexState *ls) {
  /* classstat -> CLASS NAME '{' {classfield} '}' */
  FuncState *fs = ls->fs;
  int line = ls->linenumber;
  TString *classname;
  expdesc v, b;
  ...
  /* Register as global immediately so it's visible inside methods */
  buildglobal(ls, classname, &v);
  checkglobal(ls, classname, line);

  checknext(ls, '{');

  /* Create a new table on the stack (the class) */
  reg = fs->freereg;
  luaK_codevABCk(fs, OP_NEWTABLE, reg, 0, 0, 0);
  luaK_code(fs, 0);
  init_exp(&b, VNONRELOC, reg);
  luaK_reserveregs(fs, 1);

  /* Parse class body: fields and methods */
  while (ls->t.token != '}' && ls->t.token != TK_EOS) {
    int current_reg = fs->freereg;
    /* skip optional separators */
    if (testnext(ls, ',') || testnext(ls, ';'))
      continue;
    if (ls->t.token == '}' || ls->t.token == TK_EOS) break;

    if (ls->t.token == TK_FUNCTION) {  /* method definition */
      int methline = ls->linenumber;
      expdesc key, val, tfield;
      luaX_next(ls);  /* skip 'function' */
      codestring(&key, str_checkname(ls));
      tfield = b;
      luaK_indexed(fs, &tfield, &key);
      classfuncbody(ls, &val, methline);  /* user provides 'self' explicitly */
      luaK_storevar(fs, &tfield, &val);
    }
    else if (ls->t.token == TK_NAME) {  /* field: NAME = expr */
      expdesc key, val, tfield;
      codestring(&key, str_checkname(ls));
      checknext(ls, '=');
      tfield = b;
      ...
      luaX_syntaxerror(ls, "unexpected token in class body");
    }
    fs->freereg = (lu_byte)current_reg;  /* reset registers after each field/method */
  }
  checknext(ls, '}');

  /* Set class.__index = class  (self-referential for OOP inheritance) */
  {
    expdesc key, tfield, tabcopy;
    codestring(&key, luaS_newliteral(ls->L, "__index"));
    tfield = b;
    luaK_indexed(fs, &tfield, &key);
    tabcopy = b;
    luaK_exp2anyregup(fs, &tabcopy);
    luaK_storevar(fs, &tfield, &tabcopy);
  }

  /* Assign the table to the global variable */
  luaK_storevar(fs, &v, &b);
  luaK_fixline(fs, line);
}


static void emustat (LexState *ls) {
  /* emustat -> EMU NAME '{' NAME {',' NAME} '}' */
  FuncState *fs = ls->fs;
  TString *enumname;
  expdesc v, b;
  int i = 0;
  int line = ls->linenumber;
  luaX_next(ls);  /* skip EMU */
  enumname = str_checkname(ls);
  checknext(ls, '{');

  /* Create a table on the stack top */
  luaK_codevABCk(fs, OP_NEWTABLE, fs->freereg, 0, 0, 0);
  luaK_code(fs, 0); /* extra arg */
  init_exp(&b, VNONRELOC, fs->freereg);
  luaK_reserveregs(fs, 1);
  ...
  /* Assign the table to a global variable with the enum name */
  buildglobal(ls, enumname, &v);
  checkglobal(ls, enumname, line);
  luaK_storevar(fs, &v, &b);
  luaK_fixline(fs, line);
}
