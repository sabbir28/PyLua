...
static void check_match(LexState *ls, int what, int who, int where) {
  if (l_unlikely(!testnext(ls, what))) {
    if (where == ls->linenumber) /* all in the same line? */
      ...
      luaX_syntaxerror(ls,
                       luaO_pushfstring(ls->L, "%s expected (to close %s at line %d)",
...
static short registerlocalvar(LexState *ls, FuncState *fs, TString *varname) {
  ...
  luaM_growvector(ls->L, f->locvars, fs->ndebugvars, f->sizelocvars, LocVar, SHRT_MAX,
                  "local variables");
...
static int new_varkind(LexState *ls, TString *name, lu_byte kind) {
  ...
  Dyndata *dyd = ls->dyd;
  Vardesc *var;
  luaM_growvector(L, dyd->actvar.arr, dyd->actvar.n + 1, dyd->actvar.size, Vardesc,
                  SHRT_MAX, "variable declarations");
...
static void check_readonly(LexState *ls, expdesc *e) {
  ...
  if (varname)
    luaK_semerror(ls, "attempt to assign to const variable '%s'", getstr(varname));
}
...
static l_noret undefgoto(LexState *ls, Labeldesc *gt) {
  ...
  luaK_semerror(ls, "no visible label '%s' for <goto> at line %d", getstr(gt->name),
                gt->line);
}
...
static void statlist(LexState *ls) {
  ...
  while (!block_follow(ls, 1)) {
    if (ls->t.token == TK_RETURN || ls->t.token == TK_RETUN) {
      statement(ls);
...
static void check_block_separator(LexState *ls, int keyword) {
  if (testnext(ls, ':'))
    return;
  checknext(ls, keyword);
}

static void optional_colon(LexState *ls) { testnext(ls, ':'); }
...
static void whilestat(LexState *ls, int line) {
  ...
  condexit = cond(ls);
  enterblock(fs, &bl, 1);
  check_block_separator(ls, TK_DO);
  block(ls);
...
static void test_then_block(LexState *ls, int *escapelist) {
  ...
  check_block_separator(ls, TK_THEN);
  block(ls); /* 'then' part */
  if (ls->t.token == TK_ELSE || ls->t.token == TK_ELSEIF ||
      ls->t.token == TK_ELIF)                   /* followed by 'else'/'elseif'? */
...
static void ifstat(LexState *ls, int line) {
  ...
  while (ls->t.token == TK_ELSEIF || ls->t.token == TK_ELIF)
    test_then_block(ls, &escapelist); /* ELSEIF/ELIF cond THEN/: block */
...
static void pythondefstat(LexState *ls, int line) {
  /* pythondefstat -> DEF (funcname | NAME) body
     Inside a Python-style class block, a bare name is stored as
     ClassName.name. */
  int ismethod = 0;
  expdesc v, b;
  luaX_next(ls); /* skip DEF */
  if (ls->pyclass != NULL && luaX_lookahead(ls) == '(') {
    TString *fname = str_checkname(ls);
    expdesc key;
    buildvar(ls, ls->pyclass, &v);
...
static void code_empty_table(LexState *ls, expdesc *t) {
  FuncState *fs = ls->fs;
  int pc = luaK_codevABCk(fs, OP_NEWTABLE, 0, 0, 0, 0);
  luaK_code(fs, 0); /* space for extra arg. */
...
static void pythonclassstat(LexState *ls, int line) {
  /* pythonclassstat -> CLASS NAME ':' block END
     The class name is initialized to a table. Bare DEF statements inside
     the body become fields on that table. */
  expdesc v, t;
  TString *classname;
  TString *oldclass = ls->pyclass;
  luaX_next(ls); /* skip CLASS */
  classname = str_checkname(ls);
  buildvar(ls, classname, &v);
  check_readonly(ls, &v);
  checknext(ls, ':');
  code_empty_table(ls, &t);
  luaK_storevar(ls->fs, &v, &t);
  luaK_fixline(ls->fs, line);
  ls->pyclass = classname;
  block(ls);
  ls->pyclass = oldclass;
...
static void statement(LexState *ls) {
  ...
  case TK_DEF: { /* stat -> pythondefstat */
    pythondefstat(ls, line);
    break;
  }
  case TK_CLASS: { /* stat -> pythonclassstat */
    pythonclassstat(ls, line);
    break;
  }
  ...
  case TK_RETURN:
  case TK_RETUN: { /* stat -> retstat */
    luaX_next(ls); /* skip RETURN/RETUN */
    retstat(ls);
    break;
  }
...
LClosure *luaY_parser(lua_State *L, ZIO *z, Table *anchor, Mbuffer *buff, Dyndata *dyd,
