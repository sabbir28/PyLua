...
/* Lua Wrapper */
static int luaH_md5(lua_State *L) {
    size_t len;
    ...
    MD5Init(&ctx);
    MD5Update(&ctx, (const unsigned char *)input, (unsigned int)len);
    MD5Final(digest, &ctx);
...
static const luaL_Reg hashlit_funcs[] = {
    {"md5", luaH_md5},
    {NULL, NULL}
};

LUAMOD_API int luaopen_hash(lua_State *L) {
    luaL_newlib(L, hashlit_funcs);
    return 1;
}
