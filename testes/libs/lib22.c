...
static int newstr(lua_State *L) {
  ...
  struct STR *blk = (struct STR *)allocf(ud, NULL, 0, len + 1 + sizeof(struct STR));
...
static void initstr(lua_State *L) {
  ...
  lua_pushstring(L, "012345678901234567890123456789012345678901234567890123456789");
  lua_call(L, 1, 1);              /* call newstr("0123...") */
  luaL_ref(L, LUA_REGISTRYINDEX); /* keep string in the registry */
}

static const struct luaL_Reg funcs[] = {{"id", id}, {"newstr", newstr}, {NULL, NULL}};
